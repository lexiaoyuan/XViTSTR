...
def grad_rollout(attentions, gradients, discard_ratio):
    ...
    for res in result_list:
        mask1 = res.mean(dim=1)[0, 1:]
        width1 = int(mask1.size(-1)**0.5)
        mask1 = mask1.reshape(width1, width1).numpy()
...
class VITAttentionGradRollout:
    ...
    def __call__(self, input_tensor, category_index, converter):
        self.model.zero_grad()
        label = ''
        target = converter.encode(label)
        output = self.model(input_tensor, text=target,
                            seqlen=converter.batch_max_length)
        category_mask = torch.zeros(output.size()).to(
